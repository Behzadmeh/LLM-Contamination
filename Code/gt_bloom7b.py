...
for myseed in seeds:
    ...
    roundName = f'GrounTruth_bloom7b_Seed{myseed}'
    ...
    for myrow in range(df.shape[0]):
        ...
        df.col[df.shape[0]-1, 'full_question'] = f"Question: {df.col[df.shape[0]-1, 'stem']} (A) {df.col[df.shape[0]-1, 'AA'] } (B) {df.col[df.shape[0]-1, 'BB'] } (C) {df.col[df.shape[0]-1, 'CC'] } (D) {df.col[df.shape[0]-1, 'DD'] }. \nThe correct response is: ({df.col[df.shape[0]-1, 'answerKey'] }) {df.col[df.shape[0]-1, 'newGold'] } "
